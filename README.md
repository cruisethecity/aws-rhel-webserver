...
Verified public access via AWS Public IP, demonstrating successful web server configuration and DNS routing. !(website-proof.png)

2. Security Configuration
Verified internal firewall rules using firewall-cmd. This demonstrates that the http service was specifically allowed through the OS kernel firewall. !(firewall-proof.png)
...
Key Commands Usedbash
1. Elevate to Root
sudo -i

2. System Update & Install
dnf update -y dnf install httpd firewalld -y

3. Enable Services
systemctl enable --now httpd systemctl enable --now firewalld

4. Configure Internal Firewall
firewall-cmd --permanent --add-service=http firewall-cmd --reload

5. Create Content
echo '<h1>Hello Kansas City!</h1><p>I built this server on AWS RHEL 9.</p>' > /var/www/html/index.html
